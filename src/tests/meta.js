...
export default function (setup) {
  ...
  describe('Meta information', () => {
    it('gets meta information for a component', () => {
      return setup({components: [{componentId: 'a', version: '1.0.0', ports: [{ port: 'in', kind: 'input', type: 'generic' }]}], meta: {a: {x: [{value: 'y', version: '1.0.0'}]}}})
        .then((app) =>
          chai.request(app)
          .get('/meta/a/x')
        )
    ...
    it('uses latest meta information', () => {
      return setup({
        components: [{componentId: 'a', version: '1.0.0', ports: [{ port: 'in', kind: 'input', type: 'generic' }]}, {componentId: 'a', version: '0.8.0', ports: [{ port: 'in', kind: 'input', type: 'generic' }]}],
    ...
    it('can query all meta keys for a component', () => {
      return setup({
        components: [{componentId: 'a', version: '0.1.0', ports: [{ port: 'in', kind: 'input', type: 'generic' }]}],
    ...
    it('sets meta information for a component', () => {
      return setup({components: [{componentId: 'a', version: '0.1.0', ports: [{ port: 'in', kind: 'input', type: 'generic' }]}], meta: {}})
        .then((app) =>
          chai.request(app)
          .post('/meta/a/x')
          .send({value: 'z'})
    ...
    it('sets meta information for the latest version of the component', () => {
      return setup({components: [{componentId: 'a', version: '0.1.0', ports: [{ port: 'in', kind: 'input', type: 'generic' }]}, {componentId: 'a', version: '0.2.0', ports: [{ port: 'in', kind: 'input', type: 'generic' }]}], meta: {}})
        .then((app) =>
          chai.request(app)
            .post('/meta/a/x')
            .send({value: 'z'})
    ...
    it('fails to get non-existing meta information', () => {
      return setup({components: [{componentId: 'a', version: '0.1.0', ports: [{ port: 'in', kind: 'input', type: 'generic' }]}], meta: {a: {x: [{value: 'y', version: '0.1.0'}]}}})
        .then((app) => {
